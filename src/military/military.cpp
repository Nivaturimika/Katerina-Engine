...
namespace military {
	...
	uint32_t naval_supply_from_naval_base(sys::state& state, dcon::province_id prov, dcon::nation_id nation) {
		uint32_t supply = uint32_t(state.defines.naval_base_supply_score_base * (std::pow(2, (dcon::fatten(state.world, prov).get_building_level(state.economy_definitions.naval_base_building) -1))));
		if(dcon::fatten(state.world, prov).get_building_level(state.economy_definitions.naval_base_building) != 0) {
			return supply;
		} else {
			...
			for(auto si : state.world.nation_get_state_ownership(n)) {
				...
				for(auto p : state.world.state_definition_get_abstract_state_membership(d)) {
					if(p.get_province().get_nation_from_province_ownership() == n) {
						...
						nb_level = std::max(nb_level, int32_t(p.get_province().get_building_level(state.economy_definitions.naval_base_building)));
						if(nb_level > 0)
	...
	int32_t province_point_cost(sys::state& state, dcon::province_id p, dcon::nation_id n) {
		...
		*/
		...
		if(!state.world.province_get_is_colonial(p)) {
			total += state.world.province_get_building_level(p, state.economy_definitions.naval_base_building);
		}
		auto fac_range = state.world.province_get_factory_location(p);
		total += int32_t(fac_range.end() - fac_range.begin());
		total += state.world.province_get_building_level(p, state.economy_definitions.fort_building);
	...
	void implement_war_goal(sys::state& state, dcon::war_id war, dcon::cb_type_id wargoal, dcon::nation_id from,
			...
			if((bits & cb_flag::all_allowed_states) == 0) {
				for(auto prov : state.world.state_definition_get_abstract_state_membership(wargoal_state)) {
					if(prov.get_province().get_nation_from_province_ownership() == target) {
						prov.get_province().set_building_level(state.economy_definitions.fort_building, 0);
					}
				}
				...
				for(auto si : state.world.nation_get_state_ownership(target)) {
					if(trigger::evaluate(state, allowed_states, trigger::to_generic(si.get_state().id), trigger::to_generic(from),
						...
						province::for_each_province_in_state_instance(state, si.get_state(),
							[&](dcon::province_id prov) { state.world.province_set_building_level(prov, state.economy_definitions.fort_building, 0); });
					}
				}
			}
		}
		...
		if((bits & cb_flag::po_destroy_naval_bases) != 0) {
			if((bits & cb_flag::all_allowed_states) == 0) {
				for(auto prov : state.world.state_definition_get_abstract_state_membership(wargoal_state)) {
					if(prov.get_province().get_nation_from_province_ownership() == target) {
						prov.get_province().set_building_level(state.economy_definitions.naval_base_building, 0);
					}
				}
			} else if(auto allowed_states = state.world.cb_type_get_allowed_states(wargoal); allowed_states) {
				for(auto si : state.world.nation_get_state_ownership(target)) {
					if(trigger::evaluate(state, allowed_states, trigger::to_generic(si.get_state().id), trigger::to_generic(from),
							 trigger::to_generic(from))) {
						province::for_each_province_in_state_instance(state, si.get_state(),
							[&](dcon::province_id prov) { state.world.province_set_building_level(prov, state.economy_definitions.naval_base_building, 0); });
					}
				}
			}
		}
	...
	float effective_army_speed(sys::state& state, dcon::army_id a) {
		...
		*/
		...
		return min_speed * (state.world.army_get_is_retreating(a) ? 2.0f : 1.0f) *
			(1.0f + state.world.province_get_building_level(state.world.army_get_location_from_army_location(a), state.economy_definitions.railroad_building)
	...
	float greatest_adjacent_hostile_fort_level(sys::state& state, dcon::nation_id nation_as, dcon::province_id prov) {
		...
		for(auto adj : state.world.province_get_province_adjacency(prov)) {
			if((adj.get_type() & (province::border::impassible_bit | province::border::coastal_bit)) == 0) {
				...
				auto const level = other.get_building_level(state.economy_definitions.fort_building);
	...
	float total_army_weight(sys::state& state, dcon::army_id a) {
		auto ar = dcon::fatten(state.world, a);
		float weight = 0.f;
		if(!ar.get_black_flag() && !ar.get_is_retreating()
		&& !ar.get_navy_from_army_transport()
		&& !ar.get_battle_from_army_battle_participation()) {
			for(auto rg : ar.get_army_membership()) {
				weight += 3.0f * rg.get_regiment().get_strength();
			}
		}
		return weight;
	}
	float local_army_weight(sys::state& state, dcon::province_id prov) {
		...
		for(auto ar : state.world.province_get_army_location(prov)) {
			weight += total_army_weight(state, ar.get_army());
		}
		return weight;
	}

	float max_supply_weight_in_province(sys::state& state, dcon::nation_id n, dcon::province_id prov) {
		auto const prov_attrition_mod = state.world.province_get_modifier_values(prov, sys::provincial_mod_offsets::attrition);
		auto const max_attrition = std::max(0.001f, state.world.province_get_modifier_values(prov, sys::provincial_mod_offsets::max_attrition));
		auto const supply_limit = supply_limit_in_province(state, n, prov);
		auto const attrition_mod = 1.0f + state.world.nation_get_modifier_values(n, sys::national_mod_offsets::land_attrition);
		auto const greatest_hostile_fort = greatest_adjacent_hostile_fort_level(state, n, prov);
		return supply_limit + prov_attrition_mod + greatest_hostile_fort;
	}
	...
	float weighted_attrition_amount(sys::state& state, dcon::nation_id nation_as, dcon::province_id prov, float weight) {
		auto const prov_attrition_mod = state.world.province_get_modifier_values(prov, sys::provincial_mod_offsets::attrition);
		auto const max_attrition = std::max(0.001f, state.world.province_get_modifier_values(prov, sys::provincial_mod_offsets::max_attrition));
		auto const supply_limit = supply_limit_in_province(state, nation_as, prov);
		auto const attrition_mod = 1.0f + state.world.nation_get_modifier_values(nation_as, sys::national_mod_offsets::land_attrition);
		auto const greatest_hostile_fort = greatest_adjacent_hostile_fort_level(state, nation_as, prov);
		auto const owned_by_nation = state.world.province_get_nation_from_province_ownership(prov) == nation_as;
		auto const attrition_value =
			std::clamp(weight * attrition_mod - (supply_limit + prov_attrition_mod + greatest_hostile_fort), 0.0f, max_attrition)
			+ state.world.province_get_siege_progress(prov) > 0.f ? state.defines.siege_attrition : 0.f
			+ (!owned_by_nation && !province::any_adjacent_is_friendly(state, nation_as, prov) ? 3.f : 0.f);
	...
	float relative_attrition_amount(sys::state& state, dcon::army_id a, dcon::province_id prov) {
		auto const weight = local_army_weight(state, prov);
		auto const army_controller = state.world.army_get_controller_from_army_control(a);
		return weighted_attrition_amount(state, army_controller, prov, weight);
	}
	...
	void apply_attrition(sys::state& state) {
		concurrency::parallel_for(0, state.province_definitions.first_sea_province.index(), [&](int32_t i) {
			...
			auto const weight = military::local_army_weight(state, prov);
			/*
			...
			*/
			...
			for(auto ar : state.world.province_get_army_location(prov)) {
				...
				&& !bool(ar.get_army().get_battle_from_army_battle_participation())) {
					auto const army_controller = ar.get_army().get_controller_from_army_control();
					auto const supply_limit = supply_limit_in_province(state, army_controller, prov);
					auto const attrition_mod = 1.0f + army_controller.get_modifier_values(sys::national_mod_offsets::land_attrition);
					auto const greatest_hostile_fort = greatest_adjacent_hostile_fort_level(state, army_controller, prov);
					auto const owned_by_nation = state.world.province_get_nation_from_province_ownership(prov) == army_controller;
					auto const attrition_value =
						std::clamp(weight * attrition_mod - (supply_limit + prov_attrition_mod + greatest_hostile_fort), 0.0f, max_attrition)
						+ state.world.province_get_siege_progress(prov) > 0.f ? state.defines.siege_attrition : 0.0f
						+ (!owned_by_nation && !province::any_adjacent_is_friendly(state, army_controller, prov) ? 3.f : 0.f);
	...
	void update_land_battles(sys::state& state) {
		...
		concurrency::parallel_for(0, int32_t(isize), [&](int32_t index) {
			...
			*/
			...
			int32_t crossing_adjustment = (crossing_value == defender_bonus_crossing_none ? 0 : (crossing_value == defender_bonus_crossing_river ? -1 : -2));
			...
			if((!attacking_nation && local_control) ||
				...
				defender_fort = 1.0f + 0.1f * state.world.province_get_building_level(location, state.economy_definitions.fort_building);
			}
		...
		dcon::province_id prov{dcon::province_id::value_base_t(id)};
			...
			if(total_sieging_strength == 0.0f) {
				...
				*/
				...
				int32_t fort_level = state.world.province_get_building_level(prov, state.economy_definitions.fort_building);
		...
		for(uint32_t count = schedule_array.size(); count-- > 0;) {
			...
			*/
			auto province_speed = state.defines.mobilization_speed_base
				...
				* (state.world.province_get_building_level(schedule_array[count].where, state.economy_definitions.railroad_building)) / 5.0f);
