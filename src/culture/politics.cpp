				...
				for(int32_t i = start; i < end; i++) {
					...
					&& state.world.political_party_get_ideology(pid) == iid) {
						...
						auto popt = state.world.political_party_get_party_issues(pid, dcon::issue_id{ dcon::issue_id::value_base_t(j) });
							...
							if(opt_mod) {
								party_special_issues_support_total += trigger::evaluate_additive_modifier(state, opt_mod, trigger::to_generic(nation), trigger::to_generic(nation), 0);
								count_found += 1.0f;
							}
						}
				...
				for(int32_t i = start; i < end; i++) {
					...
					&& state.world.political_party_get_ideology(pid) == iid) {
						...
						auto popt = state.world.political_party_get_party_issues(pid, dcon::issue_id{ dcon::issue_id::value_base_t(j) });
							...
							if(opt_mod) {
								party_special_issues_support_total += trigger::evaluate_additive_modifier(state, opt_mod, trigger::to_generic(n), trigger::to_generic(n), 0);
								count_found += 1.0f;
							}
						}
