...
GLuint create_program(std::string_view vertex_shader, std::string_view fragment_shader) {
	...
	auto lib_v_shader = compile_shader(
		...
		"\t\tnew_world_pos.x = cos(angle_x) * sin(angle_y);\n"
		"\t\tnew_world_pos.y = sin(angle_x) * sin(angle_y);\n"
		"\t\tnew_world_pos.z = cos(angle_y);\n"
		"\t\tnew_world_pos.xyz = rotation * new_world_pos.xyz + rotation * new_world_pos.xyz * world_pos.y;\n"
		"\t\tnew_world_pos.w *= (1.f + world_pos.y);\n"
		"\t\treturn model_proj_view * new_world_pos;\n"
		"}\n"
		"//Inner globe coords\n"
		"vec4 inner_globe_coords(vec3 world_pos) {\n"
		"\t\tvec4 new_world_pos = vec4(1.f);\n"
		"\t\tfloat angle_x = 2.f * world_pos.x * PI;\n"
		"\t\tfloat angle_y = world_pos.z * PI;\n"
		"\t\tnew_world_pos.x = cos(angle_x) * sin(angle_y);\n"
		"\t\tnew_world_pos.y = sin(angle_x) * sin(angle_y);\n"
		"\t\tnew_world_pos.z = cos(angle_y);\n"
		"\t\tnew_world_pos.xyz = rotation * new_world_pos.xyz;\n"
		"\t\tnew_world_pos.x *= (1.f + world_pos.y);\n"
		"\t\tnew_world_pos.y *= (1.f + world_pos.y);\n"
		"\t\tnew_world_pos.z *= (1.f + world_pos.y);\n"
		"\t\tnew_world_pos.yz = new_world_pos.zy;\n"
		"\t\treturn model_proj_view * new_world_pos;\n"
		"}\n"
		"//Perspective coords\n"
		"vec4 perspective_coords(vec3 world_pos) {\n"
		"\t\tvec4 new_world_pos = vec4(1.f);\n"
		"\tfloat angle_x = world_pos.x * 2.f * PI;\n"
		...
		"\treturn vec4(v.x, v.y, clamp(v.z, -1.f, 1.f), v.w);\n"
		"}\n"
		"vec4 calc_gl_position(in vec3 world_pos) {\n"
		"\tswitch(int(subroutines_index)) {\n"
		"\t\tcase 0: return globe_coords(world_pos);\n"
		"\t\tcase 1: return perspective_coords(vec3(world_pos.x, world_pos.z, 0.f));\n"
