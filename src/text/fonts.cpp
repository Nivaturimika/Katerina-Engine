...
namespace text {
		...
		if(U_SUCCESS(errorCode)) {
			...
			if(U_SUCCESS(errorCode)) {
				for(int32_t i = 0; i < runcount; ++i) {
					...
					hb_buffer_pre_allocate(hb_buf, static_cast<unsigned int>(source.size()));
	...
	void font::remake_cache(stored_glyphs& txt, std::string const& s) {
		...
		hb_buffer_clear_contents(hb_buf);
		hb_buffer_pre_allocate(hb_buf, static_cast<unsigned int>(s.size()));
			...
			if(U_SUCCESS(errorCode)) {
				...
				if(U_SUCCESS(errorCode)) {
					for(int32_t i = 0; i < runcount; ++i) {
						...
						// shape run with harfbuzz
						hb_buffer_clear_contents(hb_buf);
						hb_buffer_pre_allocate(hb_buf, static_cast<unsigned int>(temp_text.size()));
