...
namespace ui {
	...
	xy_pair sort_template_offset{};
		...
		void on_create(sys::state& state) noexcept override {
			...
			auto ptr = make_element_by_type<factory_production_sort<production_sort_order::primary_workers>>(state,
			...
			auto ptr2 = make_element_by_type<factory_production_sort<production_sort_order::secondary_workers>>(state,
				state.ui_state.defs_by_name.find(state.lookup_key("sort_by_pop_template"))->second.definition);
			...
			auto ptr3 =
				make_element_by_type<factory_production_sort<production_sort_order::owners>>(state,
				state.ui_state.defs_by_name.find(state.lookup_key("sort_by_pop_template"))->second.definition);
