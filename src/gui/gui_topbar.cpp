...
namespace ui {
	...
	dcon::province_id topbar_colony_icon::get_state_def_province(sys::state& state, dcon::state_definition_id sdef) noexcept {
	...
	void topbar_colony_icon::button_action(sys::state& state) noexcept {
	...
	int32_t topbar_colony_icon::get_icon_frame(sys::state& state, dcon::nation_id nation_id) noexcept {
	...
	void topbar_colony_icon::update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept {
		auto nation_id = retrieve<dcon::nation_id>(state, parent);
		auto nation_fat_id = dcon::fatten(state.world, nation_id);
		bool is_empty = true;
		for(auto si : state.world.nation_get_state_ownership(nation_id)) {
	...
	void topbar_rebels_icon::on_update(sys::state& state) noexcept {
	...
	void topbar_rebels_icon::update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept {
	...
	void topbar_rebels_icon::button_action(sys::state& state) noexcept {
	...
	int32_t topbar_ongoing_election_icon::get_icon_frame(sys::state& state, dcon::nation_id nation_id) noexcept {
		return int32_t(!politics::is_election_ongoing(state, nation_id));
	}

	void topbar_ongoing_election_icon::update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept {
		auto nation_id = retrieve<dcon::nation_id>(state, parent);
		auto box = text::open_layout_box(contents, 0);
	...
	void topbar_building_factories_icon::update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept {
		auto nation_id = retrieve<dcon::nation_id>(state, parent);
		auto box = text::open_layout_box(contents, 0);
