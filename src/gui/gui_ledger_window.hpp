...
namespace ui {
	...
	class ledger_nation_population_buttons : public window_element_base {
		...
		void on_create(sys::state& state) noexcept override {
			...
			auto button_def = state.ui_state.defs_by_name.find(state.lookup_key("ledger_default_button"))->second.definition;
			auto ibutton_def = state.ui_state.defs_by_name.find(state.lookup_key("ledger_pop_icon"))->second.definition;
	...
	class ledger_province_listbox : public listbox_element_base<ledger_province_entry, dcon::province_id> {
		...
		void on_update(sys::state& state) noexcept override {
			row_contents.clear();
			for(auto po : state.world.nation_get_province_ownership(state.local_player_nation)) {
				row_contents.push_back(po.get_province());
			}
	...
	class ledger_province_population_entry : public listbox_row_element_base<dcon::province_id> {
		...
		state.world.for_each_pop_type([&](dcon::pop_type_id id) { ++num_pop_types; });
			...
			// For each pop type generate
			state.world.for_each_pop_type([&](dcon::pop_type_id id) {
				auto ptr = make_element_by_type<province_population_per_pop_type_text>(state, state.ui_state.defs_by_name.find(state.lookup_key("ledger_default_textbox"))->second.definition, id);
				apply_offset(ptr);
				add_child_to_front(std::move(ptr));
			});
		}
