...
float full_pop_spending_cost(sys::state& state, dcon::nation_id n) {
	...
	total *= (0.25f / state.defines.alice_needs_scaling_factor);
	assert(std::isfinite(total) && total >= 0.0f);
	return total;
}
...
void daily_update(sys::state& state, bool initiate_buildings) {
	...
	*/
	...
	state.world.execute_parallel_over_pop([&](auto ids) {
		...
		auto adj_pop_of_type = pop_of_type * (0.25f / state.defines.alice_needs_scaling_factor);
		...
		acc_u = acc_u + ve::select(none_of_above && state.world.pop_type_get_has_unemployment(types),
			s_spending * (pop_of_type - employment) * (0.25f / state.defines.alice_needs_scaling_factor) * unemp_level * ln_costs, 0.0f);
...
float estimate_pop_payouts_by_income_type(sys::state& state, dcon::nation_id n, culture::income_type in) {
	...
	return total * (0.25f / state.defines.alice_needs_scaling_factor);
}
