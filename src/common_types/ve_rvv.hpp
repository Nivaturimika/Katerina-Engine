...
// Originally for x86/64 only, now extended for RISCV64 too!
//
...
namespace ve {
	...
	struct alignas(__m128) mask_vector {
		...
		RELEASE_INLINE bool operator[](uint32_t i) const noexcept {
			...
			_mm_store_si128(&u.tmp, _mm_castps_si128(value));
			return u.lane[i] != 0;
		}
	...
	struct alignas(__m128i) tagged_vector {
		...
		RELEASE_INLINE tag_type operator[](uint32_t i) const noexcept {
			if constexpr(tag_type::zero_is_null_t::value) {
				...
				_mm_store_si128(&u.tmp, value);
				r.value = typename tag_type::value_base_t(u.lane[i] + 1);
				return r;
			} else {
				union {
					__m128i tmp;
				...
				_mm_store_si128(&u.tmp, value);
				return tag_type(typename tag_type::value_base_t(u.lane[i]));
			}
		}
	...
	RELEASE_INLINE int_vector operator*(int_vector a, int_vector b) {
		...
		_mm_store_si128(&u[0].tmp, a.value);
		_mm_store_si128(&u[1].tmp, b.value);
		r.lane[0] = int32_t(int64_t(u[0].lane[0]) * int64_t(u[1].lane[0]));
		r.lane[1] = int32_t(int64_t(u[0].lane[1]) * int64_t(u[1].lane[1]));
		r.lane[2] = int32_t(int64_t(u[0].lane[2]) * int64_t(u[1].lane[2]));
		r.lane[3] = int32_t(int64_t(u[0].lane[3]) * int64_t(u[1].lane[3]));
		return r.tmp;
	}
	...
	RELEASE_INLINE int_vector operator<<(int_vector a, int_vector b) {
		return _mm_sll_epi32(a.value, b.value);
	}
	RELEASE_INLINE int_vector operator>>(int_vector a, int_vector b) {
		return _mm_srl_epi32(a.value, b.value);
	}
	...
	// Definitions of select are earlier than usual
	RELEASE_INLINE fp_vector select(mask_vector mask, fp_vector a, fp_vector b) {
		__m128 t1 = _mm_and_ps(a, mask);
		__m128 t2 = _mm_andnot_ps(mask, b);
		return _mm_or_ps(t1, t2);
	}
	RELEASE_INLINE int_vector select(mask_vector mask, int_vector a, int_vector b) {
		__m128i t1 = _mm_and_si128(a.value, _mm_castps_si128(mask));
		__m128i t2 = _mm_andnot_si128(_mm_castps_si128(mask), b.value);
	...
	RELEASE_INLINE tagged_vector<T> select(mask_vector mask, tagged_vector<T> a, tagged_vector<typename ve_identity<T>::type> b) {
		__m128i t1 = _mm_and_si128(a, _mm_castps_si128(mask));
		__m128i t2 = _mm_andnot_si128(_mm_castps_si128(mask), b);
		return tagged_vector<T>(_mm_or_si128(t1, t2), std::true_type{});
	}

	RELEASE_INLINE fp_vector min(fp_vector a, fp_vector b) {
		return ve::select(_mm_cmplt_ps(a, b), a, b);
	}
	RELEASE_INLINE fp_vector max(fp_vector a, fp_vector b) {
		return ve::select(_mm_cmpgt_ps(a, b), a, b);
	}
	...
	// Select is usually defined here but it has to be defined earlier
