...
namespace parsers {
	...
	void building_file::result(std::string_view name, building_definition&& res, error_handler& err, int32_t line, scenario_building_context& context) {
		...
		if(res.type == "factory") {
			...
			auto max_level = res.max_level;
			if(max_level == 0) {
				err.accumulated_errors += "Max level not defined for " + std::string(name) + " (" + err.file_name + ")\n";
				max_level = 99;
			}
			context.state.world.factory_type_set_max_level(factory_id, max_level);
