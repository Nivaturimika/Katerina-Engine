// From DataContainer project
// https://raw.githubusercontent.com/schombert/DataContainer/refs/heads/master/Catch_ve_tests/Catch_ve_tests.cpp
...
// TODO
	...
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx(fveca[i] + fvecb[i]));
	}
	fres = 4.0f + fvecb;
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx(4.0f + fvecb[i]));
	}
	fres = fveca * fvecb;
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx(fveca[i] * fvecb[i]));
	...
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx(1.0f / (fvecb[i])).margin(0.001));
	}
	fres = ve::sqrt(fvecb);
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx(std::sqrt( (fvecb[i]))));
	}
	fres = ve::inverse_sqrt(fvecb);
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx(1.0f / std::sqrt((fvecb[i]))).margin(0.001));
	}
	fres = ve::abs(fvecb);
	for(int32_t i = 0; i < ve::vector_size; ++i) {
	...
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx( (fveca[i] * (fveca[i] + fvecb[i])) + fvecb[i] ));
	}
	...
	for(int32_t i = 0; i < ve::vector_size; ++i) {
		REQUIRE(fres[i] == Catch::Approx(float(ivecb[i])));
	}
...
TEST_CASE("loads and stores", "[ve_tests]") {
	...
	int8_t reference_int8_data[] = { 1, 23, 5, 1, -3, 5, 0, 1, 5, 23, 5, 2, 6, int8_t(235), 23, 35, int8_t(235), int8_t(253), 36, int8_t(186), 86,
		100, 101, 102, 104, 102, 104, 34, int8_t(234), 88, 77, 66 };
